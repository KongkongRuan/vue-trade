<template>
  <div>
    <el-input style="width:600px" placeholder="请输入模糊搜索内容" v-model="input" class="input-with-select">
      <el-select style="width:120px" v-model="select" slot="prepend" placeholder="请选择">
        <el-option label="用户名" value="1"></el-option>
        <el-option label="用户ID" value="2"></el-option>
      </el-select>
      <el-button slot="append" @click="criteriaQuery" icon="el-icon-search"></el-button>
    </el-input>
    <el-table
      v-loading="loading"
    ...
    >
      ...
      <el-table-column prop="userType" label="权限" width="250">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.userType==3">管理员</el-tag>
          <el-tag type="success" v-if="scope.row.userType==2">普通用户</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="qq" label="QQ" width="240"></el-table-column>
      <el-table-column prop="schoolId" label="学校" width="280">
        <template slot-scope="scope">
          <span v-if="scope.row.schoolId==1">郑州工商学院</span>
        </template>
      </el-table-column>

      <el-table-column prop="enableStatus" label="帐号状态" width="200">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.enableStatus==1" type="success">正常</el-tag>
          <el-tag v-if="scope.row.enableStatus==-1" type="danger">封禁</el-tag>
        </template>
      </el-table-column>

      <el-table-column width="200" prop="enableStatus" label="操作">
        <template slot-scope="scope">
          <el-button-group>
            <el-button
              type="primary"
              @click="banned(scope.row.userId)"
              v-if="scope.row.enableStatus!=-1&&scope.row.userType!=3"
            >封停</el-button>
            <el-button
              type="primary"
