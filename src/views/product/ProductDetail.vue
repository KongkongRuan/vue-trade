<template>
  <div>
    ...
    <div style="  margin-left: 20px;margin-right: 20px;margin-top: 10px;">
      <van-row>
        <van-col span="3"><img style="height:40px;width:40px"
               :src="Common.ServerBasePath+'/trade/upload/headPortrait/head.jpg'"></van-col>
        <van-col span="12">
          <div class="username">{{seller.name}}<img v-if="seller.sex=='男'" style="height:18px;width:18px;margin-left:5px;"
             :src="Common.ServerBasePath+'/trade/upload/headPortrait/man.png'"><img v-if="seller.sex=='女'" style="height:18px;width:18px;margin-left:5px;"
             :src="Common.ServerBasePath+'/trade/upload/headPortrait/woman.png'"></div>
      ...
      <van-divider />
      <div>
        <span style="font-size: 14px;color: red;">￥</span><span style="font-size: 18px;color: red;font-weight: bold;">{{product.price}}</span>
        <span v-if="product.originalPrice!=0"
              style="font-size: 12px;padding-left: 14px;text-decoration:line-through;color: #999;">{{"￥"+product.originalPrice}}</span>
        <el-tag v-if="product.freight==0"
                size="mini"
                style="margin-left: 10px;">包邮</el-tag>
        <el-tag v-if="product.freight!=0"
                size="mini"
                type="info"
                style="margin-left: 10px;">不包邮</el-tag>
      </div>

      <div style="margin-top: 20px;word-wrap:break-word">{{product.productDesc}}</div>
      <div style="margin-top: 20px;"
           v-for="productImg in productImgList"
           :key="productImg">
        <img style="width:100%"
      ...
      <div class="font1">
        ...
        <span style="margin-left:5px;margin-bottom:5px">担保交易</span>
        <span class="font2">赞 {{praiseCount}}·浏览 {{product.browse}}</span>
      </div>
      <van-divider />
      <!-- 管理 -->
      <van-dialog v-model="managementShow"
                  :show-cancel-button="false"
                  :show-confirm-button="false">
        <van-cell><span style="margin-left: 42%;">管理我的商品</span></van-cell>
        <van-cell v-if="product.enableStatus!=2" @click="edit(product)"><span style="margin-left: 45%;color:rgb(25,137,250)">编辑</span></van-cell>
        <van-cell @click="deleteProduct(product)"><span style="margin-left: 45%;color:rgb(25,137,250)">删除</span></van-cell>
        <van-cell @click="cancel"><span style="margin-left: 45%;color:rgb(25,137,250)">取消</span></van-cell>
      </van-dialog>
      ...
      <van-goods-action>
        ...
        <van-goods-action-icon v-if="productPraise.isPraise"
                               @click="praise">
          <van-icon style="margin-bottom:5px"
                    name="good-job"
                    size="18"
                    color="#1989fa" /><span style="margin-bottom:5px">赞</span> </van-goods-action-icon>
        ...
        <!-- <van-goods-action-icon icon="chat-o"
                               text="留言"
                               @click="onClickIcon" /> -->
        <van-goods-action-icon v-if="productCollect.isCollect"
                               @click="collect">
          <van-icon style="margin-bottom:5px"
                    name="star"
                    size="18"
                    color="rgb(255,218,51)" /><span style="margin-bottom:5px">收藏</span> </van-goods-action-icon>

        <van-goods-action-icon v-if="!productCollect.isCollect"
                               icon="star-o"
                               text="收藏"
                               @click="collect" />
        <van-goods-action-button v-if="seller.userId!=buyer.userId && product.enableStatus==1"
                                 type="danger"
                                 text="立即购买"
                                 @click="createOrder" />
                                 
        <van-goods-action-button v-if="product.enableStatus==2"
          ...
          <van-button type="default"
                      ...
                      style="margin-left: 30px;margin-right: 30px;margin-top: 9px;float:right;float：right;">卖掉了</van-button>
        </van-goods-action-button>

        <van-goods-action-button v-if="seller.userId==buyer.userId"
                                 style="margin-bottom:10px;"
                                 @click="management">
          <van-button type="default"
                      plain
                      ...
                      round
                      style="margin-right: 30px;margin-top: 9px;float:right;">管理</van-button>
        </van-goods-action-button>
